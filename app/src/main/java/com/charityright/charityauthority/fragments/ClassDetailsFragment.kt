...
class ClassDetailsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        classViewModel.classDetailsResponse.observe(viewLifecycleOwner, Observer{

            Picasso.get().load("${BaseUrl.URL}${it?.data?.image}").placeholder(R.drawable.image_placeholder).into(binding.coverImg)
            ...
            if (it?.data?.student_list?.isNotEmpty() == true){
                ...
                studentAdapter = StudentAdapter(findNavController(),"null", it.data?.student_list ?: emptyList())
