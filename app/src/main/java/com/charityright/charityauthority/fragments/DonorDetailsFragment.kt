...
class DonorDetailsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        viewModel.donorDetailsResponse.observe(viewLifecycleOwner, Observer{
            if (it?.response_status == "200"){

                Picasso.get().load("${BaseUrl.URL}${it.data?.image_url}").placeholder(R.drawable.image_placeholder).error(R.drawable.image_placeholder).into(binding.imageView)
