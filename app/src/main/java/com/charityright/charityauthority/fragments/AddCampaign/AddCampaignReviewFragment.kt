...
class AddCampaignReviewFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        detailsViewModel.campaignDetailsResponse.observe(viewLifecycleOwner, Observer{
            if(it?.response_status == "200"){
                Picasso.get().load("${BaseUrl.URL}${it.data?.cover_img}").fit().centerCrop().placeholder(R.drawable.image_placeholder).into(binding.imageView)
