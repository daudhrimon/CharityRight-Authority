...
class BeneficiaryViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private suspend fun getImageUploadResponse(updateImage: MultipartBody.Part): ImageUploadModel {
        val response = AuditorRequestInterface().uploadImage("Bearer "+ CustomSharedPref.read("TOKEN",""),updateImage)
    ...
    private suspend fun getBeneficiaryResponse(): BaseResponse {
        val response = AuditorRequestInterface().addBeneficiary("Bearer "+ CustomSharedPref.read("TOKEN",""),title,name,age,location,additional,remarks,images)
