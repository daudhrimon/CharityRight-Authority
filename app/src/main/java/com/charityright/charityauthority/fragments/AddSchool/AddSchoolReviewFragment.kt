...
class AddSchoolReviewFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        viewModel.schoolDetailsResponse.observe(viewLifecycleOwner, Observer{

            imageLayoutManager = LinearLayoutManager(requireContext(),LinearLayoutManager.HORIZONTAL,false)
            classImageAdapter = ClassImageAdapter(it?.data?.images ?: emptyList(),viewModel)
        ...
        viewModel.schoolVisitReportResponse.observe(viewLifecycleOwner, Observer{
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.getImagePath().observe(viewLifecycleOwner, Observer{
            Picasso.get().load("${BaseUrl.URL}$it").fit().centerCrop().placeholder(R.drawable.image_placeholder).into(binding.imageView)
        })
    }
