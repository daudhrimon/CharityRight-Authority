...
class AddSchoolFragmentViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private suspend fun getAddSchoolResponse(): AddSchoolBaseResponse {
        val response = AdminRequestInterface().addSchool("Bearer "+ CustomSharedPref.read("TOKEN",""),zone_id,school_name,address,details,lat.toString(),lon.toString(),class_list,images)
