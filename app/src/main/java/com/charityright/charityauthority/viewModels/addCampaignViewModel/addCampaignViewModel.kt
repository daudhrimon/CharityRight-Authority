...
class addCampaignViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private suspend fun getAddCampaignResponse(): campaignDetailsBaseReponse {
        val response = AdminRequestInterface().addCampaignData("Bearer "+ CustomSharedPref.read("TOKEN",""),zone_id, title, date, location, details, objective, beneficiary, m_name, m_contact, images)
    ...
    private suspend fun getAddImageResponse(updateImage: MultipartBody.Part): AddSchoolImageBaseResponse {
        val response = AdminRequestInterface().addSchoolImage("Bearer "+ CustomSharedPref.read("TOKEN",""),updateImage)
